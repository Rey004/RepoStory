...
function BadgeRow({ icon: Icon, label, isLightMode }) {
  return (
    <div className={`flex items-center gap-1.5 py-1 border-b ${isLightMode ? "border-zinc-100/60" : "border-zinc-900/40"} last:border-b-0 text-[9px] font-mono`}>
      <Icon className={`w-3.5 h-3.5 shrink-0 ${isLightMode ? "text-amber-500" : "text-green-level-4"}`} />
      <span className={`truncate ${isLightMode ? "text-zinc-650" : "text-zinc-350"}`}>{label}</span>
    </div>
  );
}

export default function Achievements({ repoDetails, storyData, contributorsCount, releasesCount, isLightMode, isNested = false }) {
      ...
      {isNested ? (
          ...
          {achievements.map((item, idx) => {
            ...
            return (
              <div key={item} className="flex items-start gap-1 min-w-0 text-[7.5px] font-mono leading-tight py-0.5">
                <BadgeIcon className={`w-2.5 h-2.5 shrink-0 ${isLightMode ? "text-emerald-600" : "text-green-level-4"} mt-[2.5px]`} />
                <span className={`break-words ${isLightMode ? "text-zinc-900" : "text-white"}`}>{item}</span>
              </div>
            );
          })}
          {achievements.length === 0 && (
            <div className="col-span-2 flex items-center gap-1.5 text-[7.5px] font-mono text-zinc-550 py-1">
        ...
        <>
          <div className="flex items-center justify-between gap-2">
            <span className={`text-[9px] uppercase tracking-wider font-mono ${isLightMode ? "text-zinc-500" : "text-zinc-500"}`}>earned</span>
            ...
            {achievements.map((item) => (
              ...
              >
                {(() => {
                  ...
                  return <BadgeIcon className={`w-3.5 h-3.5 ${isLightMode ? "text-emerald-600" : "text-green-level-4"}`} />;
                })()}
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            <span className={`px-2.5 py-1 rounded-full text-[9px] font-mono uppercase tracking-wider border ${isLightMode ? "bg-white border-emerald-200 text-emerald-700" : "bg-black/50 border-green-level-4/20 text-green-level-4"}`}>
              quests complete
            </span>
