...
export default function CardHeader({ repoDetails, archetype, isLightMode, textSub, description, readmeSummary, languageList }) {
  return (
    <div
      className={`relative z-10 flex flex-col gap-4 pb-4 mb-4 border-b ${
        isLightMode ? "border-zinc-200" : "border-green-level-4/15"
      }`}
    >
      <div className="flex items-start justify-between gap-4">
        <div className="flex items-center gap-3 min-w-0 flex-1">
          <div className={`relative shrink-0 p-0.5 rounded-full ${isLightMode ? "bg-zinc-100" : "bg-green-level-4/10"}`}>
            <img
